...
class MapPage(tk.Frame):
    def __init__(self, parent, controller=None):
        ...
        try:
            self.map_widget.set_position(50.45, 30.52)
        ...
        tk.Label(
            ...
            bg=BG_COLOR,
            font=("Consolas", 12),
        ).pack(pady=(10, 0), anchor="w", padx=4)
        ...
        self.edit_btn = tk.Button(
            btn_frame,
            text="Edit",
            command=lambda: self.edit_or_delete("edit"),
            bg=BG_COLOR,
            fg=TEXT_COLOR,
            font=FONT,
            relief="flat",
        ...
        self.del_btn = tk.Button(
            btn_frame,
            text="Delete",
            command=lambda: self.edit_or_delete("delete"),
            bg=BG_COLOR,
            fg=TEXT_COLOR,
            font=FONT,
            relief="flat",
        ...
        tk.Button(
            ...
            font=FONT,
            relief="flat",
        ).pack(fill="x", pady=12, side="bottom")
    ...
    def _on_canvas_click(self, event):
        ...
        if not conv:
            ...
            self.adding = False
            self.info.config(text="Status: idle")
            return
        try:
    ...
    def on_listbox_double(self, event):
        ...
        if sel:
            ...
            try:
                self.map_widget.set_position(m["lat"], m["lon"])
            except Exception:
                pass
    ...
    def edit_or_delete(self, action):
        sel = self.listbox.curselection()
        if not sel:
            messagebox.showinfo("Info", f"Select a marker to {action}")
            return
        idx = sel[0]
        m = self.markers[idx]
        if action == "edit":
    ...
    def save_markers(self):
        try:
            with open(MARKERS_FILE, "w", encoding="utf-8") as f:
                json.dump(
                    [{"lat": m["lat"], "lon": m["lon"], "text": m["text"]} for m in self.markers],
                    f,
                    ensure_ascii=False,
                    indent=2,
                )
        ...
        if os.path.exists(MARKERS_FILE):
            try:
                ...
                for item in data:
                    ...
                    self.markers.append(
                        {"lat": lat, "lon": lon, "text": item.get("text", ""), "obj": None}
                    )
                self.reload_markers()
            except Exception as e:
    ...
    def reload_markers(self):
        ...
        for m in self.markers:
            try:
                m["obj"] = self.map_widget.set_marker(m["lat"], m["lon"], text=m["text"])
            except Exception:
                m["obj"] = None
        self.update_listbox()
    ...
    def save_all(self):
        self.save_markers()
        self.save_settings()
        self.info.config(text="Saved.")

    def load_all(self):
        self.load_settings()
        self.load_markers()
        self.info.config(text="Loaded markers")

    def clean_data(self):
        if messagebox.askyesno("Clean data", "Delete saved map files?"):
            ...
            self.markers = []
            self.reload_markers()
            self.info.config(text="Data cleared")
...
class MapApp(tk.Tk):
    def __init__(self):
        ...
        self.page = MapPage(self, controller=None)
